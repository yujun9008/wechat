      ...
      this.state.checkedList.map((record,index)=>{
        (function(){
          nextProps.dataList.map((item)=>{
            if(record.callLegionCode == item.callLegionCode){
  ...
  render() {
    ...
    let contentbody = this.state.checkedList.map((item)=>{
      ...
      quotaCofig.map((record)=>{
        if(record.fieldAdd){
            tds.push(
              <div key={item.callLegionCode + record.field}>
          ...
          if(record.field == "legionName"){
            //如果是事业部 加头部删除按钮
            tds.push(
              <div key={item.callLegionCode + record.field}>
                <Icon type="cross-circle-o" onClick={()=>(this.handleTitleClick(item.callLegionCode))}/>
